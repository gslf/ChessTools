...
class TestPGNToImage(unittest.TestCase):
    def setUp(self):
        ...
        self.output_filename = "final_position"
    ...
    def test_render_from_pgn_string(self):
        ...
        render_from_pgn_string(pgn_string, self.theme, self.output_dir.name, self.output_filename, final_position_only=True)

        path = self.output_dir.name + f"\{self.output_filename}.png"
    ...
    def test_render_from_pgn_file(self):
        ...
        render_from_pgn_file(tmp_pgn_path, self.theme, self.output_dir.name, self.output_filename, final_position_only=True)

        path = os.path.join(self.output_dir.name, f"{self.output_filename}.png")
        self.assertTrue(os.path.exists(path))  
        os.remove(path)
    ...
    def test_render_from_pgn_file_not_exist(self):
        ...
        with self.assertRaises(FileNotFoundError):
            render_from_pgn_file("non_existing.pgn", self.theme, self.output_dir.name, self.output_filename, final_position_only=True)
    ...
    @mock.patch("image_processing.pgn_to_image.render_from_pgn_file")
    def test_render_from_pgn_folder(self, mock_render_from_pgn_file, mock_listdir):
        ...
        render_from_pgn_folder(folder_path, self.theme, self.output_dir.name, final_position_only=True)
    ...
    def test_render_from_pgn_folder_not_exist(self):
        ...
        with self.assertRaises(FileNotFoundError):
            render_from_pgn_folder("non_existing_folder", self.theme, self.output_dir.name, final_position_only=True)
